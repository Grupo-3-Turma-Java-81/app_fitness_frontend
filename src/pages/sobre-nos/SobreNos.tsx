...
function SobreNos() {
    ...
    const prevSlide = () => {
        setCurrentIndex((prevIndex) =>
            (prevIndex - 1 + equipe.length) % equipe.length
        );
    };
        ...
        <div className="min-h-screen bg-black p-8 pb-20">
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <div className="flex flex-col gap-8 text-justify">
                            <p className="max-w-3xl text-white leading-relaxed text-lg">
                                O sistema da Pulso é simples e eficiente para o cadastro e consulta de alunos, além do registro de seus respectivos treinos. Voltado para o uso interno de academias, substitui métodos manuais de organização dos alunos por uma solução digital ágil e segura.
                            </p>
                            ...
                            <div className="max-w-3xl bg-zinc-700 p-6 rounded-lg border-2 border-[#D7FF00] shadow-lg hover:shadow-xl transition-all duration-300">
                                ...
                                <p className="leading-relaxed text-white">
                                    Automatizar o gerenciamento de dados básicos dos alunos da academia, melhorando
            ...
            <div className="max-w-full mx-auto px-4 py-8">
                <div className="relative max-w-6xl mx-auto">
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 group-hover:text-[#D7FF00]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="flex justify-center gap-8 transition-all duration-500 ease-in-out">
                        {[0, 1].map((offset) => {
                            const index = (currentIndex + offset) % equipe.length;
                            const member = equipe[index];
                            return (
                                <div
                                    key={member.nome}
                                    className="bg-zinc-800 rounded-2xl overflow-hidden shadow-xl transform transition-all duration-300 hover:scale-105 hover:shadow-2xl w-[320px]"
                                >
                                    ...
                                    <div className="p-6">
                                        <h3 className="text-2xl font-bold text-white mb-2">{member.nome}</h3>
                                        <div className="flex items-center gap-3 mb-4">
                                            <div className="h-px w-8 bg-gradient-to-r from-[#D7FF00] to-transparent"></div>
                                            <p className="text-[#D7FF00] font-medium">{member.descricao}</p>
                                        </div>
                                        <p className="text-white/80 leading-relaxed text-sm">{member.sobre}</p>
                                    </div>
                                </div>
                            );
                        })}
